...
function Items() { 
    ...
    useEffect(() => {
      const getitems= async()=>{
        await  axios.get(`http://localhost:4000/getitems/${title}`).then((response) => {
           console.log(response.data.recordset);
           setFoods(response.data.recordset)
         }); 
     }
    getitems();
      ...
      const handlereplenish = async(item)=>{
        console.log(item)
        const res = await axiosPrivate.post('http://localhost:4000/replenish',
        {
          item : item,
        },
        {
    ...
    if(res.data.success)
    {
       toast.success("Stock has been replenished !",styles)
    }
      }

      const handleSearch = async(e)=>{
        setSearch(e.target.value)
        const res = await axiosPrivate.post(`http://localhost:4000/filteritems/${title}`,
        {
 ...
 <div class="flex-grow text-gray-800">
  ...
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
    ...
    className={`flex ${title=='Eatables'?'bg-red-700':'bg-[#f91944]'}  cursor-pointer hover:bg-red-700  w-32 px-[25px] py-2 items-start text-lg justify-start   rounded-full  text-white`}
    >
        Eatables
    </div>
    <div
    onClick={()=>{navigate('/items/Beverages')}}
    className={`flex ${title=='Beverages'?'bg-red-700':'bg-[#f91944]'}  cursor-pointer hover:bg-red-700  w-32 px-[16px] py-2 items-start text-lg justify-start   rounded-full  text-white`}
    >
        Beverages
    </div>
    <div
    onClick={()=>{navigate('/items/Seafood')}}
      className={`flex ${title=='Seafood'?'bg-red-700':'bg-[#f91944]'}  cursor-pointer hover:bg-red-700  w-32 px-[25px] py-2 items-start text-lg justify-start   rounded-full  text-white`}
    >
        Seafood
    </div>
    </div>
      <section class="z-0 grid md:grid-cols-2  place-content-center xl:grid-cols-3 gap-6">
      ...
      {foods?.map((food) => (
       ...
       <button 
       onClick={()=>{handlereplenish(food.ItemID)}}
       disabled = {food.CurrentStock!==0}
       className={`flex mb-4 py-1 w-28 items-center justify-center ${food.CurrentStock ===0 ? "bg-red-600 text-white cursor-pointer border-1 font-bold hover:bg-red-800 rounded-lg":"bg-red-100  border-[1px] rounded-full border-[#f91944] text-[#f91944]" }  `}>{food.CurrentStock !== 0?food.CategoryName:"Replenish"}</button>
        ...
        <div class="flex justify-between items-center">
          <div>
            ...
            <p class="mt-2 font-bold"><span>&#8377;</span>{food.Price}</p>
          </div>
          <div>
            <button
            disabled = {food.CurrentStock === 0}
            onClick={()=>{navigate(`/fooditem/${food.ItemName}`)}}
            class="text-white text-md font-semibold bg-[#f91944] hover:bg-red-700 py-2 px-4 rounded-lg shadow-md hover:shadow-lg">{food.CurrentStock === 0 ? "Out Of Stock" : food.CurrentStock > food.ThresholdStock ? "Add" : `${food.CurrentStock + " Left"}`}</button>
          </div>
        </div>
       </div>
       </div>
